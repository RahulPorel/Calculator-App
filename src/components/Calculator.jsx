...
const Calculator = () => {
  const [isDarkMode, setIsDarkMode] = useState(
    JSON.parse(localStorage.getItem("cal-dark-mode")) || false
  );
  const [expression, setExpression] = useState("");
  const [result, setResult] = useState("");
  const [history, setHistory] = useState(
    JSON.parse(localStorage.getItem("cal-history")) || []
  );
  const [toggleHistory, setToggleHistory] = useState(false);

  useEffect(() => {
    localStorage.setItem("cal-dark-mode", JSON.stringify(isDarkMode));
  }, [isDarkMode]);

  useEffect(() => {
    localStorage.setItem("cal-history", JSON.stringify(history));
  }, [history]);
    ...
    >
      <div className="app_calculator">
        ...
        <div className="del">
          {" "}
          &nbsp;
          <p onClick={handleHistoryChange} className="history">
            {" "}
            🕐
          </p>{" "}
          &nbsp;&nbsp;
          <i className="fa-solid fa-clock-rotate-left"></i>
          <p onClick={handleDel}>AC</p>
        </div>
